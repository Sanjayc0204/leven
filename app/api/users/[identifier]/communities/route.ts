...
import { getUserCommunities, getUserCommunitiesByEmail} from '@/services/userService';
...
export async function GET(req: NextRequest, { params }: { params: { identifier: string } }): Promise<NextResponse> {
  ...
  if (!identifier) {
    return new NextResponse(JSON.stringify({ success: false, error: 'Identifier is required' }), { status: 400 });
  }
